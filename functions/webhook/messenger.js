// functions/webhook/messenger.js - POST+GET /webhook/messenger (FB Messenger bot, Pages Function)
// Instagram/Telegram wale pattern par: same brain (Gemini), same memory (KV warna in-memory).
...
const MSG_MAX_CHARS = 600; // Meta Messenger text limit ~640 chars (andaza se safe)
const MSG_MAX_BYTES = 1800; // safe UTF-8 margin
...
async function needsDisclosure(env, psid) {
  const key = "ms:greeted:" + psid;
  if (env.LWD_KV) {
    try {
      const raw = await env.LWD_KV.get(key);
      if (raw) return false;
...
// ---------- Messenger Send API ---------- //
async function sendMessengerMessage(pat, psid, text) {
  if (!pat) return;
  try {
...
// char AUR byte dono limit ke andar chunks (Devanagari 3 bytes/char)
...
// ---------- webhook signature verify (optional, agar MESSENGER_APP_SECRET set hai) ---------- //
...
export async function onRequestPost(context) {
  const { request, env } = context;
  const pat = env.MESSENGER_PAGE_ACCESS_TOKEN || "";
  const brain = createBrain(env);

  const rawBody = await request.text();
  if (!(await validSignature(request, rawBody, env.MESSENGER_APP_SECRET || ""))) {
    return new Response("Forbidden", { status: 403 });
  }
  ...
  for (const entry of entries) {
    const messagings = (entry.messaging || []).concat(entry.standby || []);
    for (const m of messagings) {
      // Get Started button / menu click
      if (m.postback && m.postback.payload) {
        const psid0 = m.sender && m.sender.id;
        if (psid0) await sendMessengerMessage(pat, psid0, ABOUT_TEXT);
        continue;
      }
      ...
      if (!text) {
        // attachment/sticker - abhi text hi chalta hai
        if (msg.attachments || msg.sticker_id) {
          await sendMessengerMessage(pat, psid, ATTACHMENT_REPLY);
        }
        continue;
      }

      if (rateLimited("ms:" + psid, RATE_LIMIT_CHAT)) {
        await sendMessengerMessage(pat, psid, RATE_LIMIT_REPLY);
        continue;
      }

      const memKey = "ms:" + psid;
      const history = await loadHistory(env, memKey);
      let reply = await brain.reply(text, history, "");

      if (await needsDisclosure(env, psid)) {
      ...
      for (const chunk of chunkMessage(reply)) {
        await sendMessengerMessage(pat, psid, chunk);
      }
    }
  }
  return Response.json({ ok: true });
